package com.matrix.ucenterservice.service.impl;
...
import com.matrix.ucenterservice.mapper.UcenterMemberMapper;
import com.matrix.ucenterservice.service.UcenterMemberService;
import com.matrix.ucenterservice.utils.MD5;
...
public class UcenterMemberServiceImpl extends ServiceImpl<UcenterMemberMapper, UcenterMember> implements UcenterMemberService {
    ...
    public void register(RegisterVo registerVo) {
        ...
        if(StringUtils.isEmpty(nickname)||StringUtils.isEmpty(mobile)||
            ...
            throw new MatrixException(20001,"注册信息缺失");
        }
        //2 验证手机号是否重复
        ...
        if(count>0){
            throw new MatrixException(20001,"手机号重复");
        }
        //3 验证短信验证码
        String redisCode = redisTemplate.opsForValue().get(mobile);
        if(!code.equals(redisCode)){
            throw new MatrixException(20001,"验证码错误");
        }
    ...
    public String login(LoginVo loginVo) {
        ...
        if(StringUtils.isEmpty(mobile)||StringUtils.isEmpty(password)){
            throw new MatrixException(20001,"手机号或密码有误");
        }
        //2 根据手机号获取用户信息
        QueryWrapper<UcenterMember> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("mobile",mobile);
